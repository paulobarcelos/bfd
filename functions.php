...
/*add_action( 'admin_notices', 'style_admin_style' );
function style_admin_style(){
     global $current_screen;
     if ( $current_screen->post_type == 'style' ){
...
function designer_parent_meta_box($post) {
	post_custom_parrent_meta_box( $post, 'style', 'designer Style' );
}
...
function product_parent_meta_box($post) {
	post_custom_parrent_meta_box( $post, 'category', 'Product Category' );
}
...
// Admin metaboxes
function post_custom_parrent_meta_box($post, $type, $type_name) {
	...
	if ( $post_type_object->hierarchical ) {
		...
		?>
			<select name="post_parent">
				<option>Choose the <?php echo $type_name;?></parent>
				<?php foreach ($posts as $category_post):?>
					<option value="<?php echo $category_post->ID;?>" <?php if ($current_parrent_id == $category_post->ID):?> selected <?php endif;?> ><?php echo get_the_title($category_post->ID);?></option>
				<?php endforeach;?>
			</select>
		<?php
	}
...
function test($lang_code){
	global $q_config;
	$array = array(
				'name' => 'Testing Text ('.$q_config['language_name'][$lang_code].')',
				'desc' => $q_config['language_name'][$lang_code],
				'id'   => $lang_code . 'test_text',
				'type' => 'text',
			);
	return $array;
}
